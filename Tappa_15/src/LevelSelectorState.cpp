...
LevelSelectorState::LevelSelectorState(sf::RenderWindow &window, StateManager &sm, ScenarioEditor &se, std::string title) : window(window), stateManager(sm), scenarioEditor(se), mapEditor(*static_cast<MapEditor *>(nullptr)), gameState(*static_cast<GameState *>(nullptr)), title(title)
{
    r = std::regex("[^a-zA-Z0-9!,\",-,/]");
...
LevelSelectorState::LevelSelectorState(sf::RenderWindow &window, StateManager &sm, GameState &gs, std::string title) : window(window), stateManager(sm), gameState(gs), mapEditor(*static_cast<MapEditor *>(nullptr)), scenarioEditor(*static_cast<ScenarioEditor *>(nullptr)), title(title)
{
    r = std::regex("[^a-zA-Z0-9!,\",-,/]");
...
{
    std::string path = "../../Tappa_15/resources/maps";
    ...
    for (auto &row : maps)
    {
        for (auto &map : row)
        {
            if (scenarioEditor.isInScenario(map.path))
            {
                ...
                cursor.setPosition(map.position);
                window.draw(cursor);
            }
        }
    }
...
void LevelSelectorState::moveCursorLeft()
{
    ...
    {
        cursorPosition.x = static_cast<int>(maps[page * maxRows + cursorPosition.y].size()) - 1;
        moveCursorUp();
    }
    else
    {
...
void LevelSelectorState::moveCursorDown()
{
    ...
    {
        cursorPosition.y++;
        cursorPosition.x = std::min(
            cursorPosition.x,
            static_cast<int>(maps[page * maxRows + cursorPosition.y].size()) - 1);
    }
    else
    {
        nextPage();
...
void LevelSelectorState::previousPage()
{
    ...
    {
        page--;
        int lastRowIndex = std::min(maxRows, static_cast<int>(maps.size() - page * maxRows)) - 1;

        cursorPosition.y = lastRowIndex;
        cursorPosition.x = static_cast<int>(maps[page * maxRows + lastRowIndex].size()) - 1;
    }
}
